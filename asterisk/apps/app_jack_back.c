...
static const char jack_app[] = "JACK_BACK";
	...
	struct ast_frame *out_frame)
{
	...
	for (;;) {
		...
		if (res < read_len) {
			...
			if (out_frame) {
				ast_debug(1, "Sending an empty frame for the JACK_HOOK_BACK\n");
	...
	struct ast_frame *frame, enum ast_audiohook_direction direction)
{
	...
	if (!(datastore = ast_channel_datastore_find(chan, &jack_hook_ds_info, NULL))) {
		ast_log(LOG_ERROR, "JACK_HOOK_BACK datastore not found for '%s'\n", chan->name);
		ast_channel_unlock(chan);
		return -1;
	}
...
static int enable_jack_hook(struct ast_channel *chan, char *data)
{
	...
	if ((datastore = ast_channel_datastore_find(chan, &jack_hook_ds_info, NULL))) {
		ast_log(LOG_ERROR, "JACK_HOOK_BACK already enabled for '%s'\n", chan->name);
		goto return_error;
	}
	...
	ast_audiohook_init(&jack_data->audiohook, AST_AUDIOHOOK_TYPE_MANIPULATE, "JACK_HOOK_BACK");
...
static int disable_jack_hook(struct ast_channel *chan)
{
	...
	if (!(datastore = ast_channel_datastore_find(chan, &jack_hook_ds_info, NULL))) {
		ast_channel_unlock(chan);
		ast_log(LOG_WARNING, "No JACK_HOOK_BACK found to disable\n");
		return -1;
	}
	...
	const char *value)
{
	...
	if (!strcasecmp(value, "on"))
		...
		ast_log(LOG_ERROR, "'%s' is not a valid value for JACK_HOOK_BACK()\n", value);
		res = -1;
	}
...
static struct ast_custom_function jack_hook_function = {
	...
	.syntax = "JACK_HOOK_BACK(<mode>,[options])",
	.desc =
	"   The JACK_HOOK_BACK allows turning on or off jack connectivity to this channel.\n"
	...
	"this hook should run in.  A mode must be specified when turning the JACK_HOOK_BACK.\n"
	...
	"\n"
	" Examples:\n"
	"   To turn on the JACK_HOOK_BACK,\n"
	"     Set(JACK_HOOK_BACK(manipulate,i(pure_data_0:input0)o(pure_data_0:output0))=on)\n"
	"   To turn off the JACK_HOOK_BACK,\n"
	"     Set(JACK_HOOK_BACK()=off)\n"
	"",
